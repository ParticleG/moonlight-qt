...
AutoUpdateChecker::AutoUpdateChecker(QObject *parent) : QObject(parent), m_Nam(this) {
    ...
    connect(&m_Nam, SIGNAL(finished(QNetworkReply *)), this, SLOT(handleUpdateCheckRequestFinished(QNetworkReply *)));
...
void AutoUpdateChecker::parseStringToVersionQuad(QString &string, QVector<int> &version) {
    ...
    for (const QString &component : list) { version.append(component.toInt()); }
}
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        for (QJsonValueRef updateEntry : array) {
            if (updateEntry.isObject()) {
                ...
                if (!updateObj.contains("platform") || !updateObj.contains("arch") || !updateObj.contains("version") || !updateObj.contains("browser_url")) {
                    qWarning() << "Update manifest entry missing vital field";
                    continue;
                }

                if (!updateObj["platform"].isString() || !updateObj["arch"].isString() || !updateObj["version"].isString() || !updateObj["browser_url"].isString()) {
                    qWarning() << "Update manifest entry has unexpected vital field type";
                    continue;
                }

                if (updateObj["arch"] == QSysInfo::buildCpuArchitecture() && updateObj["platform"] == getPlatform()) {
                    ...
                    for (int i = 0;; i++) {
                        ...
                        if (i < latestVersionQuad.count()) { latestVer = latestVersionQuad[i]; }
                        if (i < m_CurrentVersionQuad.count()) { currentVer = m_CurrentVersionQuad[i]; }
                        if (i >= latestVersionQuad.count() && i >= m_CurrentVersionQuad.count()) { break; }

                        if (currentVer < latestVer) {
                            qDebug() << "Update available";

                            emit onUpdateAvailable(updateObj["version"].toString(), updateObj["browser_url"].toString());
                            return;
        ...
        qWarning() << "No entry in update manifest found for current platform: " << QSysInfo::buildCpuArchitecture() << getPlatform();
    } else {
