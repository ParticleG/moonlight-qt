...
CONNECTION_LISTENER_CALLBACKS Session::k_ConnCallbacks = {Session::clStageStarting, nullptr, Session::clStageFailed, nullptr, Session::clConnectionTerminated, Session::clLogMessage, Session::clRumble, Session::clConnectionStatusUpdate};
...
void Session::clConnectionTerminated(int errorCode) {
    ...
    switch (errorCode) {
        case ML_ERROR_GRACEFUL_TERMINATION: break;
...
void Session::clRumble(unsigned short controllerNumber, unsigned short lowFreqMotor, unsigned short highFreqMotor) {
    ...
    if (s_ActiveSession->m_InputHandler != nullptr) { s_ActiveSession->m_InputHandler->rumble(controllerNumber, lowFreqMotor, highFreqMotor); }
...
void Session::clConnectionStatusUpdate(int connectionStatus) {
    ...
    if (!s_ActiveSession->m_Preferences->connectionWarnings) { return; }
    ...
    switch (connectionStatus) {
        case CONN_STATUS_POOR:
            ...
            break;
        case CONN_STATUS_OKAY: s_ActiveSession->m_OverlayManager.setOverlayState(Overlay::OverlayStatusUpdate, false); break;
    }
}

bool Session::chooseDecoder(StreamingPreferences::VideoDecoderSelection vds, SDL_Window *window, int videoFormat, int width, int height, int frameRate, bool enableVsync, bool enableFramePacing, bool testOnly, IVideoDecoder *&chosenDecoder) {
...
int Session::drSetup(int videoFormat, int width, int height, int frameRate, void *, int) {
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Video stream is %dx%dx%d (format 0x%x)", width, height, frameRate, videoFormat);
...
void Session::getDecoderInfo(SDL_Window *window, bool &isHardwareAccelerated, bool &isFullScreenOnly, QSize &maxResolution) {
    IVideoDecoder *decoder;

    if (!chooseDecoder(StreamingPreferences::VDS_AUTO, window, VIDEO_FORMAT_H264, 1920, 1080, 60, true, false, true, decoder)) {
...
bool Session::isHardwareDecodeAvailable(SDL_Window *window, StreamingPreferences::VideoDecoderSelection vds, int videoFormat, int width, int height, int frameRate) {
    IVideoDecoder *decoder;

    if (!chooseDecoder(vds, window, videoFormat, width, height, frameRate, true, false, true, decoder)) { return false; }
...
bool Session::populateDecoderProperties(SDL_Window *window) {
    ...
    if (!chooseDecoder(m_Preferences->videoDecoderSelection,
                       window,
                       m_StreamConfig.enableHdr ? VIDEO_FORMAT_H265_MAIN10 : (m_StreamConfig.supportsHevc ? VIDEO_FORMAT_H265 : VIDEO_FORMAT_H264),
...
bool Session::initialize() {
    ...
    if (!testWindow) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to create window for hardware decode test: %s", SDL_GetError());
    ...
    switch (m_Preferences->audioConfig) {
        case StreamingPreferences::AC_STEREO: m_StreamConfig.audioConfiguration = AUDIO_CONFIGURATION_STEREO; break;
        case StreamingPreferences::AC_51_SURROUND: m_StreamConfig.audioConfiguration = AUDIO_CONFIGURATION_51_SURROUND; break;
        case StreamingPreferences::AC_71_SURROUND: m_StreamConfig.audioConfiguration = AUDIO_CONFIGURATION_71_SURROUND; break;
    }
    ...
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Audio channel count: %d", CHANNEL_COUNT_FROM_AUDIO_CONFIGURATION(m_StreamConfig.audioConfiguration));
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Audio channel mask: %X", CHANNEL_MASK_FROM_AUDIO_CONFIGURATION(m_StreamConfig.audioConfiguration));

    switch (m_Preferences->videoCodecConfig) {
        case StreamingPreferences::VCC_AUTO:
            // TODO: Determine if HEVC is better depending on the decoder
            m_StreamConfig.supportsHevc = isHardwareDecodeAvailable(testWindow, m_Preferences->videoDecoderSelection, VIDEO_FORMAT_H265, m_StreamConfig.width, m_StreamConfig.height, m_StreamConfig.fps);
#ifdef Q_OS_DARWIN
            {
    ...
    switch (m_Preferences->windowMode) {
        default:
        case StreamingPreferences::WM_FULLSCREEN_DESKTOP: m_FullScreenFlag = SDL_WINDOW_FULLSCREEN_DESKTOP; break;
        case StreamingPreferences::WM_FULLSCREEN: m_FullScreenFlag = SDL_WINDOW_FULLSCREEN; break;
    }
...
bool Session::validateLaunch(SDL_Window *testWindow) {
    ...
    if (m_Preferences->absoluteMouseMode && !m_App.isAppCollectorGame) { emitLaunchWarning("Your selection to enable remote desktop mouse mode may cause problems in games."); }
    ...
    if (m_Preferences->unsupportedFps && m_StreamConfig.fps > 60) {
        ...
        if (m_Preferences->enableVsync) { emitLaunchWarning("V-sync will be disabled when streaming at a higher frame rate than the display."); }
    }

    if (m_StreamConfig.supportsHevc) {
        bool hevcForced = m_Preferences->videoCodecConfig == StreamingPreferences::VCC_FORCE_HEVC || m_Preferences->videoCodecConfig == StreamingPreferences::VCC_FORCE_HEVC_HDR;
        ...
        if (m_Preferences->videoDecoderSelection == StreamingPreferences::VDS_AUTO &&    // Force hardware decoding checked below
            ...
            !isHardwareDecodeAvailable(testWindow, m_Preferences->videoDecoderSelection, VIDEO_FORMAT_H265, m_StreamConfig.width, m_StreamConfig.height, m_StreamConfig.fps)) {
            if (hevcForced) {
    ...
    if (m_StreamConfig.enableHdr) {
        ...
        } else if (!isHardwareDecodeAvailable(testWindow, m_Preferences->videoDecoderSelection, VIDEO_FORMAT_H265_MAIN10, m_StreamConfig.width, m_StreamConfig.height, m_StreamConfig.fps)) {
    ...
    if (m_AudioDisabled) { emitLaunchWarning("Failed to open audio device. Audio will be unavailable during this session."); }

    // Check for unmapped gamepads
    if (!SdlInputHandler::getUnmappedGamepads().isEmpty()) { emitLaunchWarning("An attached gamepad has no mapping and won't be usable. Visit the Moonlight help to resolve this."); }
    ...
    if (m_Preferences->videoDecoderSelection == StreamingPreferences::VDS_FORCE_HARDWARE && !m_StreamConfig.enableHdr &&    // HEVC Main10 was already checked for hardware decode support above
        !isHardwareDecodeAvailable(testWindow, m_Preferences->videoDecoderSelection, m_StreamConfig.supportsHevc ? VIDEO_FORMAT_H265 : VIDEO_FORMAT_H264, m_StreamConfig.width, m_StreamConfig.height, m_StreamConfig.fps)) {
...
public:
    DeferredSessionCleanupTask(Session *session) : m_Session(session) {}
...
private:
    ...
    void run() override {
        ...
        bool shouldQuit = !m_Session->m_UnexpectedTermination && m_Session->m_Preferences->quitAppAfter;
...
void Session::getWindowDimensions(int &x, int &y, int &width, int &height) {
    int  displayIndex = 0;
    bool fullScreen;
    ...
    // was being displayed on.
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Qt UI screen is at (%d,%d)", m_DisplayOriginX, m_DisplayOriginY);
        for (int i = 0; i < SDL_GetNumVideoDisplays(); i++) {
            SDL_Rect displayBounds;

            if (SDL_GetDisplayBounds(i, &displayBounds) == 0) {
                if (displayBounds.x == m_DisplayOriginX && displayBounds.y == m_DisplayOriginY) {
    ...
    if (fullScreen && SDL_GetDisplayBounds(displayIndex, &usableBounds) == 0) {
        ...
        if (m_Window != nullptr) {
            ...
            if (SDL_GetWindowBordersSize(m_Window, &top, &left, &bottom, &right) == 0) {
                ...
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Unable to get window border size: %s", SDL_GetError());
            }
...
void Session::updateOptimalWindowDisplayMode() {
    ...
    if (SDL_GetDesktopDisplayMode(displayIndex, &desktopMode) == 0) {
        ...
        if (desktopMode.w < m_ActiveVideoWidth || desktopMode.h < m_ActiveVideoHeight) {
            if (!StreamUtils::getRealDesktopMode(displayIndex, &desktopMode)) { return; }
        }
    } else {
    ...
    for (int i = 0; i < SDL_GetNumDisplayModes(displayIndex); i++) {
        if (SDL_GetDisplayMode(displayIndex, i, &mode) == 0) {
            if (mode.w == desktopMode.w && mode.h == desktopMode.h && mode.refresh_rate % m_StreamConfig.fps == 0) {
                if (mode.refresh_rate > bestMode.refresh_rate) { bestMode = mode; }
            }
        }
    }
    ...
    if (bestMode.refresh_rate == 0) {
        ...
        // this case.
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "No matching refresh rate found; using desktop mode");
        bestMode = desktopMode;
    }
    ...
    if ((SDL_GetWindowFlags(m_Window) & SDL_WINDOW_FULLSCREEN_DESKTOP) == SDL_WINDOW_FULLSCREEN) {
        ...
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Chosen best display mode: %dx%dx%d", bestMode.w, bestMode.h, bestMode.refresh_rate);
    }
...
public:
    AsyncConnectionStartThread(Session *session) : QThread(nullptr), m_Session(session) {}

private:
    void run() override { m_Session->m_AsyncConnectionSuccess = m_Session->startConnectionAsync(); }
...
bool Session::startConnectionAsync() {
    ...
    Q_ASSERT(m_Computer->currentGameId == 0 || m_Computer->currentGameId == m_App.id);
    ...
    for (const NvDisplayMode &mode : m_Computer->displayModes) {
        if (mode.width == m_StreamConfig.width && mode.height == m_StreamConfig.height) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Found host supported resolution: %dx%d", mode.width, mode.height);
    ...
    try {
        ...
        if (m_Computer->currentGameId != 0) {
            ...
            http.launchApp(m_App.id, &m_StreamConfig, enableGameOptimizations, prefs.playAudioOnHost, m_InputHandler->getAttachedGamepadMask());
        }
    ...
    LiInitializeServerInformation(&m_ServerInfo);
    m_ServerInfo.portBoth0            = m_NetworkPreferences->portBoth0;
    m_ServerInfo.portTcp0             = m_NetworkPreferences->portTcp0;
    m_ServerInfo.portTcp1             = m_NetworkPreferences->portTcp1;
    m_ServerInfo.portUdp0             = m_NetworkPreferences->portUdp0;
    m_ServerInfo.portUdp1             = m_NetworkPreferences->portUdp1;
    m_ServerInfo.portUdp2             = m_NetworkPreferences->portUdp2;
    m_ServerInfo.address              = hostnameStr.data();
    m_ServerInfo.serverInfoAppVersion = siAppVersion.data();

    // Older GFE versions didn't have this field
    QByteArray siGfeVersion;
    if (!m_Computer->gfeVersion.isEmpty()) { siGfeVersion = m_Computer->gfeVersion.toLatin1(); }
    if (!siGfeVersion.isEmpty()) { m_ServerInfo.serverInfoGfeVersion = siGfeVersion.data(); }
    ...
    if (m_Preferences->packetSize != 0) {
        ...
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Using custom packet size: %d bytes", m_Preferences->packetSize);
    } else {
    ...
    int err = LiStartConnection(&m_ServerInfo, &m_StreamConfig, &k_ConnCallbacks, &m_VideoCallbacks, m_AudioDisabled ? nullptr : &m_AudioCallbacks, NULL, 0, NULL, 0);
    if (err != 0) {
...
void Session::exec(int displayOriginX, int displayOriginY) {
    ...
    StreamingPreferences prefs;
    m_InputHandler = new SdlInputHandler(prefs, m_Computer, m_StreamConfig.width, m_StreamConfig.height);
    ...
    while (!asyncConnThread.wait(10)) { QCoreApplication::processEvents(QEventLoop::ExcludeUserInputEvents); }
    ...
    m_Window = SDL_CreateWindow("Moonlight", x, y, width, height, SDL_WINDOW_ALLOW_HIGHDPI);
    if (!m_Window) {
    ...
    SDL_Surface *iconSurface = SDL_CreateRGBSurfaceWithFormatFrom((void *)svgImage.constBits(), svgImage.width(), svgImage.height(), 32, 4 * svgImage.width(), SDL_PIXELFORMAT_RGBA32);
#ifndef Q_OS_DARWIN
    ...
    if (QGuiApplication::platformName() == "eglfs") { QGuiApplication::setOverrideCursor(QCursor(Qt::BlankCursor)); }
    ...
    for (;;) {
        ...
        switch (event.type) {
            case SDL_QUIT: SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Quit event received"); goto DispatchDeferredCleanup;

            case SDL_USEREVENT:
                switch (event.user.code) {
                    case SDL_CODE_FRAME_READY: m_VideoDecoder->renderFrameOnMainThread(); break;
                    case SDL_CODE_HIDE_CURSOR: SDL_ShowCursor(SDL_DISABLE); break;
                    case SDL_CODE_SHOW_CURSOR: SDL_ShowCursor(SDL_ENABLE); break;
            ...
            case SDL_WINDOWEVENT:
                ...
                if (event.window.event != SDL_WINDOWEVENT_SIZE_CHANGED && event.window.event != SDL_WINDOWEVENT_SHOWN) {
                    ...
                    if (SDL_GetWindowDisplayIndex(m_Window) == currentDisplayIndex) { break; }
                }
                ...
                SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Recreating renderer for window event: %d (%d %d)", event.window.event, event.window.data1, event.window.data2);
            ...
            case SDL_RENDER_TARGETS_RESET:
...

                {
                    ...
                    if (displayHz + 5 < m_StreamConfig.fps) {
                        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Disabling V-sync because refresh rate limit exceeded");
                        enableVsync = false;
                    }
            ...
            case SDL_KEYUP:
            case SDL_KEYDOWN: m_InputHandler->handleKeyEvent(&event.key); break;
            case SDL_MOUSEBUTTONDOWN:
            case SDL_MOUSEBUTTONUP: m_InputHandler->handleMouseButtonEvent(&event.button); break;
            case SDL_MOUSEMOTION: m_InputHandler->handleMouseMotionEvent(&event.motion); break;
            case SDL_MOUSEWHEEL: m_InputHandler->handleMouseWheelEvent(&event.wheel); break;
            case SDL_CONTROLLERAXISMOTION: m_InputHandler->handleControllerAxisEvent(&event.caxis); break;
            case SDL_CONTROLLERBUTTONDOWN:
            case SDL_CONTROLLERBUTTONUP: m_InputHandler->handleControllerButtonEvent(&event.cbutton); break;
            case SDL_CONTROLLERDEVICEADDED:
            case SDL_CONTROLLERDEVICEREMOVED: m_InputHandler->handleControllerDeviceEvent(&event.cdevice); break;
            case SDL_JOYDEVICEADDED: m_InputHandler->handleJoystickArrivalEvent(&event.jdevice); break;
            case SDL_FINGERDOWN:
            case SDL_FINGERMOTION:
            case SDL_FINGERUP: m_InputHandler->handleTouchFingerEvent(&event.tfinger); break;
        }
    }
...
DispatchDeferredCleanup:
    ...
    if (QGuiApplication::platformName() == "eglfs") { QGuiApplication::restoreOverrideCursor(); }
    ...
    if (iconSurface != nullptr) { SDL_FreeSurface(iconSurface); }
